...
$query=mysqli_query($conn,"INSERT INTO `pelamar` (`id`, `nama`, `tempat`, `tanggal`, `jk`, `alamat`, `agama`, `status`, `nohp`, `email`, `pendidikan`, `namasekolah`, `pekerjaan`,`cv`)
                ...
                if($query){
                    ...
                    echo "<script>alert('FILE GAGAL DI UPLOAD!');window.location='../formulir.php'</script>";
                }
            }
            else{
                echo "<script>alert('UKURAN FILE TERLALU BESAR!');window.location='../formulir.php'</script>";
            }
        }
        else{
            echo "<script>alert('EKSTENSI FILE YANG DI UPLOAD TIDAK DI PERBOLEHKAN!');window.location='../formulir.php'</script>";
        }
    }                                   
